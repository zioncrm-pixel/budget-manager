...
const autoAssignFromPreviousMonth = async () => {
    if (isAutoAssigning.value || isAssignmentSubmitting.value) {
        return
    }
    ...
    try {
        const response = await httpClient.post(route('budgets.manage.transactions.auto_assign'), {
            year: selectedYear.value,
            month: selectedMonth.value,
        })

        const assigned = Number(response?.data?.assigned ?? 0)
        assignmentFeedback.value = response?.data?.message || ''
        ...
        assignmentFeedback.value = 'אירעה שגיאה בשיוך האוטומטי.'
    }
                        ...
                        <div v-if="hasCategories" class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                            ...
                            >        
                                ...
                                <button class="text-sm font-semibold text-indigo-600 transition-colors hover:text-indigo-800" @click.stop="openTransactionsModal(category)">
                                    ...
                                    <div
                                        v-if="category.has_excluded_source_transactions"
                                        class="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-800"
                                    >
                                        שים לב: יש עסקאות ממקורות שלא נספרים בסיכום הכללי.
                                    </div>
                                ...
                                <div class="lg:col-span-1">
                                    <div class="flex h-full min-h-0 flex-col gap-1 rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
                                        ...
                                        <div class="mt-3 flex justify-end">
                                            <button
                                                type="button"
                                                class="inline-flex items-center justify-center rounded-md bg-indigo-600 px-3 py-2 text-sm font-semibold text-white transition hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-60"
                                                :disabled="isAutoAssigning || isAssignmentSubmitting"
                                                @click="autoAssignFromPreviousMonth"
                                            >
                                                <svg
                                                    v-if="isAutoAssigning"
